...
const routineSurface = [sessionHelper, tokenHelper, reviewerPrompt, readme].join("\n");
...
test("reviewer session helper preserves required environment and private key guards", () => {
  ...
  for (const forbiddenWrite of [
    ...
    assert.equal(
      `${sessionHelper}\n${tokenHelper}`.includes(forbiddenWrite),
      false,
...
test("reviewer session routine keeps manual dispatcher handoff verification and cleanup", () => {
  for (const expected of [
    ...
    "numbered one-command prep flow",
    "1. Verify required reviewer environment variables",
    "2. Verify private key path",
    "3. Check existing GH_TOKEN",
    "4. Generate short-lived Reviewer App installation token",
    "5. Set GH_TOKEN in this PowerShell session",
    "6. Run safe verification commands manually",
    "7. Paste the Reviewer App Dispatcher prompt manually",
    "9. Clear GH_TOKEN after Reviewer App review work",
    "The existing value will not be printed, cleared, persisted, or written to disk.",
    "This GH_TOKEN is a GitHub App installation token, not the normal GitHub user identity.",
...
test("reviewer session routine preserves Reviewer App authority boundaries", () => {
  for (const expected of [
    ...
    "8. Respect Reviewer App authority boundaries",
    "Reviewer App cannot:",
    "merge PRs",
